...
function testGamebarGetSingleActivity(
    activityID: number,
    user: i.connectionData,
    onComplete: (data: i.activityGETData) => void
) {
    const options = {
        hostname: 'api3.gamebus.eu',
...
function testGamebarGetLimitedActivities(
    startDate: string,
    endDate: string,
    limit: number,
    user: i.connectionData,
...
function testClientGetAllActs(client: GameBusClient, playerID: number) {
    ...
    client
        .request(path, method, body, headers, query, true, false)
