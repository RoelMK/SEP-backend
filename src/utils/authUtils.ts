...
export function createJWT(userId: string, accessToken: string, refreshToken: string): string {
    return jwt.sign({userId: userId, accessToken: accessToken, refreshToken: refreshToken}, 
        process.env.TOKEN_SECRET as string, { expiresIn: process.env.TOKEN_EXPIRES_IN, issuer: process.env.TOKEN_ISSUER, algorithm: 'HS256' });
}
